...
implementation. Secondary goal is a memory footprint small enough to make it
...
void bignum_to_string(struct bn *n, char *str, int nbytes) {
    ...
    int j = BN_ARRAY_SIZE -
            1; /* index into array - reading "MSB" first -> big-endian */
    ...
    while ((j >= 0) && (nbytes > (i + 1))) {
        ...
        i +=
            (2 *
             WORD_SIZE); /* step WORD_SIZE hex-byte(s) forward in the string. */
...
void bignum_sub(struct bn *a, struct bn *b, struct bn *c) {
    ...
    for (i = 0; i < BN_ARRAY_SIZE; ++i) {
        ...
        res = (tmp1 - tmp2);
        c->array[i] =
            (DTYPE)(res & MAX_VAL); /* "modulo number_base" == "% (number_base -
                                       1)" if number_base is 2^N */
        borrow = (res <= MAX_VAL);
    }
}
...
void bignum_pow_mod(struct bn *a, struct bn *b, struct bn *n, struct bn *res) {
...
static void _lshift_one_bit(struct bn *a) {
    ...
    for (i = (BN_ARRAY_SIZE - 1); i > 0; --i) {
        a->array[i] =
            (a->array[i] << 1) | (a->array[i - 1] >> ((8 * WORD_SIZE) - 1));
    }
    a->array[0] <<= 1;
}
